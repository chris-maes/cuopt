                                              ...
                                              lp_solution_t<i_t, f_t>& solution)
{
  ...
  if (settings.crossover && barrier_status == lp_status_t::OPTIMAL) {

    {
     ...
     {
       if (vector_norm_inf<i_t, f_t>(perturbation) == 0.0) {
         settings.log.printf("Perturbation is zero, skipping perturbation crossover\n");
       } else {
         std::vector<i_t> primal_partition(original_lp.num_cols, 0);
         std::vector<i_t> dual_partition(original_lp.num_cols, 0);
         for (f_t gamma = 1e-3; gamma >= 1e-13; gamma *= 1e-5) {
           ...
           for (i_t j = 0; j < original_lp.num_cols; j++) {
             if (primal_partition[j] == 0) { primal_variables_to_remove.push_back(j); }
           }
           ...
           for (i_t j : primal_variables_to_remove) {
             ...
             if (restricted_primal_problem.lower[j] != 0.0) {
               settings.log.printf("Primal variable %d with nonzero lower bound %e\n",
                                   j,
                                   restricted_primal_problem.lower[j]);
             }
             fixed_variables++;
           }
           ...
           for (i_t j = 0; j < restricted_primal_problem.num_cols; j++) {
             f_t perturb = (random_vector[j] / norm_random_vector) *
                           (norm_perturbation / (0.01 * restricted_primal_problem.num_cols +
                                                 std::max(1e-6, lp_solution.x[j])));
           ...
           lp_solution_t<i_t, f_t> restricted_solution(restricted_primal_problem.num_rows,
                                                       restricted_primal_problem.num_cols);
           ...
           barrier_solver_t<i_t, f_t> restricted_solver(
             restricted_primal_problem, restricted_presolve_info, restricted_solver_settings);
           std::vector<f_t> ignored_perturbation;
           lp_status_t restricted_status = restricted_solver.solve(
             start_time, barrier_solver_settings, restricted_solution, ignored_perturbation);
           settings.log.printf("Restricted status: %d\n", restricted_status);
           if (restricted_status != lp_status_t::OPTIMAL) { continue; }
           settings.log.printf("Restricted objective: %e\n", restricted_solution.objective);

           // Check to see if it is a basic solution
           i_t num_basic = 0;
           for (i_t j = 0; j < restricted_primal_problem.num_cols; j++) {
             if (restricted_solution.x[j] >= gamma * restricted_solution.z[j]) { num_basic++; }
           }
           settings.log.printf(
             "Number of basic variables: %d m %d\n", num_basic, restricted_primal_problem.num_rows);
