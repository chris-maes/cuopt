...
bool constexpr use_gpu = false;
bool constexpr use_augmented = true;
 ...
 public:
      ...
      handle_ptr(lp.handle_ptr)
  {
    ...
    if (!use_augmented) {
      if (use_gpu) {
        ...
        raft::copy(device_A_x_values.data(),
                   device_AD.x.data(),
                   device_AD.x.size(),
    ...
    chol = std::make_unique<sparse_cholesky_cudss_t<i_t, f_t>>(settings, factorization_size);
  ...
  void form_adat(bool first_call = false)
  {
    ...
    if (num_factorizations == 0) {
      int64_t nnz;
      if (use_gpu) {
        nnz = device_ADAT.row_start.element(device_ADAT.m, device_ADAT.row_start.stream());
      } else {
        nnz = ADAT.col_start[m];
      }
      settings_.log.printf("ADAT time %.2fs\n", adat_time);
      settings_.log.printf(
        "ADAT nonzeros %e density %.2f\n",
        static_cast<float64_t>(nnz),
        static_cast<float64_t>(nnz) / (static_cast<float64_t>(m) * static_cast<float64_t>(m)));
    }
  }
                                                         ...
                                                         f_t& max_residual)
{
  ...
  if (use_augmented) {
    ...
    if (prim_err > 1e-2)
    {
      settings.log.printf("|| A * dx - r_p || %e\n", prim_err);
    }
